...
rule trim_galore_se:
    ...
    output:
        temp(RESULT_DIR / '01_trim_galore' / '{run}.trimmed.fastq.gz')
    params:
        extra = c['extra'],
    ...
    benchmark: 'benchmarks/trim_galore/{run}.log'
    wrapper: 'http://dohlee-bio.info:9193/trim-galore'

c = config['trim_galore']
rule trim_galore_pe:
    input:
        DATA_DIR / '{run}.read1.fastq.gz',
        DATA_DIR / '{run}.read2.fastq.gz',
    output:
        temp(RESULT_DIR / '01_trim_galore' / '{run}.read1.trimmed.fastq.gz'),
        temp(RESULT_DIR / '01_trim_galore' / '{run}.read2.trimmed.fastq.gz'),
    params:
        extra = c['extra'],
        # Trim low-quality ends from reads in addition to adapter removal. For RRBS samples,
        # quality trimming will be performed first, and adapter trimming is carried in a second round.
